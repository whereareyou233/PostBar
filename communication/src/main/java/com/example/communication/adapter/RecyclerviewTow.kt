...
class RecyclerviewTow (val context: Context, val List :List<RealTimeInfo>):
    RecyclerView.Adapter<RecyclerviewTow.ViewHolder>(){
    inner class ViewHolder(view : View): RecyclerView.ViewHolder(view){
        //在这里初始化你的item控件实例
        val image : ImageView = view.findViewById(R.id.image_community_tow)
        val text : TextView =view.findViewById(R.id.text_community_tow)
    }

    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): ViewHolder {
        val view= LayoutInflater.from(parent.context).inflate(R.layout.item_recycler_community_tow,parent,false)
